...
import { ElMenuOptions } from './interface'
  ...
  const pvState: ToRefs<{
    ...
    reactive({
      ...
      isActive: computed<boolean>(() =>
        pvState.items.value.some(item => item.isActive)
      ),
      isOpen: computed(() => {
      ...
      style: computed(() => {
        const style: any = {
          ...
          borderBottomColor: pvState.isActive.value
            ? root.state.activeTextColor
            : 'transparent',
          color: pvState.isActive.value
            ? root.state.activeTextColor
            : root.state.textColor
        }
      ...
      icon: computed(() =>
        (root.state.mode === 'horizontal' && parent.isRoot) ||
        (root.state.mode === 'vertical' && !root.state.collapse)
          ? 'el-icon-arrow-down'
          : 'el-icon-arrow-right'
      ),
      placement: computed(() =>
    ...
    readonly(
      reactive({
        ...
        isActive: computed(
          () =>
            state.index.value !== -1 &&
            state.index.value === root.state.activeIndex
        ),
        style: computed(() => {
          const style: any = {
            color: state.isActive.value
              ? root.state.activeTextColor
              : root.state.textColor,
            backgroundColor: root.state.backgroundColor
          }
          ...
          if (root.state.mode === 'horizontal') {
            style.borderBottomColor = state.isActive.value
              ? root.state.activeTextColor
              : 'transparent'
          }
          return style
